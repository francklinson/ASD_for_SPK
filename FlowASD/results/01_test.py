        ...
        for id_str in machine_id_list:
            ...
            for file_idx, file_path in tqdm(enumerate(test_files), total=len(test_files)):
                try:
                    ...
                    # instead of mean use median/std
                    if machine_type in ['valve', 'slider']:
                        y_pred[file_idx] = numpy.std(errors)
                    elif machine_type in ['fan', 'ToyCar', 'ToyConveyor', 'pump']:
                        y_pred[file_idx] = numpy.median(errors)
                    else:
                        raise ValueError('Unknown machine_type {}'.format(machine_type))
