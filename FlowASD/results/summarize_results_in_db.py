...
from ..secret_config_example import mongo_password
from plot_snippet_loss import extract_model_outputs
from ..config import mongo_connection_string
from ..datasets.machine_sound_dataset import all_devtest_machines
...
def extract_plot_data_baseline(mongo_db, aggregate=True):
    ...
    results = pd.DataFrame([(e1.config['machine_type'], e1.config['machine_id'], e1.metrics['eval_rocauc'][0] * 100,
...
def extract_plot_data_conv_ae(mongo_db, aggregate=True):
    ...
    # arch_params might be different for different experiments
    # results per channel multiplier should be compared to baseline
    results = pd.DataFrame([(e1.config['machine_type'], e1.config['machine_id'],
                             e1.config['arch_params']['channel_multiplier'],
                             e1.config.get('apply_normalization', False), e1.metrics['eval_rocauc'][0] * 100,
                             e1.metrics['eval_p_rocauc'][0] * 100) for e1 in experiments])
    results = results.rename(
        columns={0: 'machine_type', 1: 'machine_id', 2: 'channel_multiplier', 3: 'apply_normalization', 4: 'rocauc',
                 5: 'p_rocauc'})
...
def extract_plot_data_flows(mongo_db, aggregate=True, grouping_vars=None):
    ...
    col_names = ['run_id', 'machine_type', 'machine_id', 'rocauc', 'p_rocauc', 'apply_normalization',
                 'frames_per_snippet',
